...
public:
	...
	/*void Sort_data() {
		system("cls");
		if (head == NULL) {
			cout << "Linked list i Empty :" << endl;
			menu();
		}

		int count = 0, t_id,t_days;
	...
	*/
	...
	{ 
	 Hotel* save =head;
        ...
        fout << " ID\tName\tDate\tdate\tphone\toomtype\tdays\tprice\n\n";
	...
	void BILL(){
		...
		fstream data;
		int p;
		cout<<"ENTER YOUR PHONE NUMBER TO PRINT YOUR BILL"<<endl;
		cin>>p;
		data.open("save.txt");
		while(!data.eof()){
			if(p==first->phone){
				...
				cout<<"PHONE :"<<first->phone<<endl;
				cout<<"DATE :"<<first->date<<endl;
				cout<<"ROOM TYPE: "<<first->roomtype<<endl;
				cout << "DAYS :" << first->days << endl;
